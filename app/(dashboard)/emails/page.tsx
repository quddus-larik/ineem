"use client"
import {Surface, Typography, Label, Chip, Avatar, Description, Separator} from "@heroui/react";
...
export default function Page() {
    ...
    useEffect(() => {
        (async function () {
            await fetchEmails("after:2026/08/07 before:2026/08/10", 30);
        })();
...
// 2. Returns ONLY the name (removes the email address entirely)
    ...
    return (
        <Surface variant="default" className={"w-full flex  flex-row items-start gap-2 rounded-2xl p-2"}>
            <Surface
                className="flex w-72 h-[80svh] flex-col items-start gap-2 rounded-2xl p-2"
            >
                <Surface
                    variant="transparent"
                            ...
                            <div className="flex-1 min-w-0 flex-col">
                                ...
                                <span>
                                    <Chip variant={"soft"} color={"accent"} size={"sm"}>
                                        <Paperclip className={"size-3"}/>03
                                    </Chip>
                                </span>
                                    <span>
                                    <Description>{formatDate(email.date) || "03 March, 2024"}</Description>
                                </span>
                                </div>
                            </div>
                        </Surface>
            ...
            {/* Show Mail */}
            <Surface className="flex flex-1 h-[80svh] flex-col gap-4 rounded-2xl p-6 overflow-y-auto">
                {selectedEmail ? (
                        ...
                        <div className="flex flex-col justify-center">
                            <div className="flex items-center gap-3">
                                <Avatar size="md" variant="soft" color="accent">
                                    <Avatar.Fallback>
                                        {(selectedEmail.from || selectedEmail.sender)?.[0] || "U"}
                                    </Avatar.Fallback>
                                </Avatar>
                                <div>
                                    ...
                                    <Label className="text-sm">
                                        From: {getCleanEmail(selectedEmail.from) || "Unknown Sender"}
                                    </Label>
                                </div>
                            </div>
                            <Description className={"mt-2"}>{formatDate(selectedEmail.date)}</Description>
                        </div>
                        <Separator />
